...
const FormInputSampah = () => {
  ...
  const onAddBtnClick = event => {
    setInputList(inputList.concat(<Question idx={idx} handleInput={(val) => handleInput(val, idx)} key={inputList.length} />));
    setIdx(idx + 1)
  };
  ...
  return (
    <>
      ...
      <ButtonValid className="lead-five" style={{ marginTop: "28px" }} onClick={showOutput}>Jual</ButtonValid>
      {/* </Link> */}
    </>
  )
}
